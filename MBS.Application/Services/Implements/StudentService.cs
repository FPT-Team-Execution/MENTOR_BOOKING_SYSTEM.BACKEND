...
public class StudentService : BaseService2<StudentService>, IStudentService
{
	...
	public async Task<BaseModel<CreateStudentResponseModel, CreateStudentRequestModel>> CreateStudent(
		CreateStudentRequestModel request)
	{
		try
		{
			...
			var emailBody = _templateService.ReplaceInTemplate(emailTemplate,
				new Dictionary<string, string> { { "{Email}", newUser.Email! }, { "{Password}", request.Password } });

			await _emailService.SendEmailAsync(EmailMessage.Create(newUser.Email!, emailBody,
				"[MBS]Invite you to the System"));
	...
	public async Task<BaseModel<UpdateStudentResponseModel>> UpdateStudentProfile(UpdateStudentRequestModel request)
	{
		try
		{
			var student = await _studentRepository.GetByUserIdAsync(request.Id, include: x => x.Include(x => x.User));
